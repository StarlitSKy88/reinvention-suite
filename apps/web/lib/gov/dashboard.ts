...
const MOCK_METRICS: GovDashboardMetrics = {
...
const MOCK_SUCCESS_CASES: GovSuccessCase[] = [
  {
    id: 'case-001',
...
function getMockMetrics(query: DashboardQuery): GovDashboardMetrics {
  ...
  if (query.period) {
    return {
      ...MOCK_METRICS,
      period: {
        start: query.period.start.toISOString().split('T')[0],
        end: query.period.end.toISOString().split('T')[0],
      },
    };
  }
  return MOCK_METRICS;
